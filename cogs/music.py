...
# --------------------------------------------------------------------------
# YouTube bloquea las IPs de hosting en la nube (como Railway) pidiendo
# "Sign in to confirm you're not a bot". Esto NO es un bug del código,
# es que YouTube desconfía de esa IP. Lo que más ayuda, en orden:
#
#   1. force_ipv4 -> Railway a veces sale por IPv6, que YouTube bloquea peor.
#   2. Probar varios "player_client" (formas de hacerse pasar por otro
#      dispositivo) hasta que uno funcione.
#   3. (Opcional, recomendado si sigue fallando) subir un archivo
#      cookies.txt exportado de una cuenta de Google "de prueba" (NO tu
#      cuenta principal) al mismo lugar que este archivo. Si existe, se
#      usa automáticamente.
#
# Aun con esto, YouTube puede volver a bloquear la IP en cualquier momento
# porque cambia sus reglas seguido; si deja de funcionar del todo, hay que
# revisar si tocaría instalar un "PO token provider" (bgutil-ytdlp-pot-provider)
# en el servidor. Por ahora, esto arregla el 403/"sign in" en la mayoría de casos.
# --------------------------------------------------------------------------

COOKIES_PATH = os.path.join(os.path.dirname(__file__), "cookies.txt")

# Distintas combinaciones de "player_client" para intentar una tras otra
# si la primera falla. YouTube confía más o menos en cada una según el día.
CLIENT_ATTEMPTS = [
    ["android", "web"],
    ["tv", "web_embedded"],
    ["ios"],
...
def build_ydl_opts(player_clients: list[str]) -> dict:
    opts = {
...
class Music(commands.Cog):
    ...
    async def search(self, query: str):
        ...
        def _extract_con(player_clients: list[str]):
            opts = build_ydl_opts(player_clients)
        ...
        for player_clients in CLIENT_ATTEMPTS:
            try:
                info = await loop.run_in_executor(None, _extract_con, player_clients)
                return {"title": info.get("title", "Sin título"), "url": info["url"]}
            except Exception as e:
                ultimo_error = e
                print(f"⚠️ Falló con player_client={player_clients}: {type(e).__name__}: {e}")
                continue  # probamos la siguiente combinación

        # Si ninguna combinación funcionó, mandamos el último error hacia arriba
        # (lo captura el /play y se lo muestra al usuario)
        raise ultimo_error
